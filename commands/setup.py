import discord
from constants import checks, setup_funcs
...
ex1 = False
ex2 = False
notes = "This server setup is very much in a beta state. Any issues must be redirected to the owner, using the `contact` command."
...
Type the new name if so, otherwise type `n`.""")
    ...
    try:
        ...
        await c.send("Successfully set notifications to mentions only.")
            ...
            await c.send(f"Error while setting default notification settings:\n{e}\nPlease check my permissions. Keeping default notifications as they are now.")
            print(e)
...
NOTE: Giving full admin privileges is dangerous! Only select this if you trust your admins.
Type `y` if you want full admin privileges, or `n` for just permissions.""")

        message = await client.wait_for("message", check=lambda s: s.author.id==m.id and s.content in ['y', 'n'])
        ...
        try:
            ...
            await g.create_role(name="Admin", permissions=discord.Permissions(permissions=perm_int), hoist=True)
